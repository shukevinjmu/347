...
const sidePanelButtons = document.getElementById("side-panel-button");
const sidePanel = document.getElementById("panel");


sidePanelButtons.addEventListener("click", () =>{
      if (window.matchMedia('screen and (max-width: 1200px)').matches) {
          if(sidePanel.style.width > 1 + '%'){
              sidePanel.style.width = 0;
              sidePanel.style.height = 100 + '%';
              sidePanelButtons.style.color = 'whitesmoke';
              sidePanel.style.display = 'none';
          } else{
              sidePanel.style.display = 'block';
              sidePanel.style.height = 30 + '%';
              sidePanel.style.width = 100 + '%';
              sidePanelButtons.style.color = '#259a7a';
          }
